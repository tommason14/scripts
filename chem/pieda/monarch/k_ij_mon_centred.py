...
def full_pieda_table(piedalog):
    ...
    for line in eof(piedalog, 0.1):
        if ("I    J DL  Z    R   Q(I->J)  EIJ-EI-EJ dDIJ*VIJ    total     Ees      Eex    Ect+mix    Edisp    Gsol"
                in line):
            found = True
            cols = line.split()
            continue
...
def calc_kij(fmo0=None, fmo3=None, pieda=None):
    ...
    two_body_rc_di = pieda_df["Edisp"].sum() * CAL_TO_J
...
def plot_graph(df, show_fig = False):
    ...
    p = sns.scatterplot(x='diffs', y='k_ij', hue='groups', data=df)
    # remove legend title
...
def main(show_fig=False):
    ...
    dirs = (sp.check_output(
        "find . -maxdepth 1 -type d | grep -v '__pycache__' | tail -n +2 | sort",
        shell=True,
    ).decode("utf-8").split("\n")[:-1])
    cwd = os.getcwd()
    folders = []

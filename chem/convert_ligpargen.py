...
def is_number(
    num,
):  # A simple function to check if an argument is a number; self-explanitory
    try:
        float(num)
        return True
    except ValueError:
...
def fetchList(
    ...
    *args
    ...
    for line in openedFile:
        ...
        if (
            currentLine.isspace() and readMode is True
        ):  # In each line, check if it's purely white space
            if (
                splitString not in previousLine or skipBlankSplit is False
        ...
        if (
            currentLine[0] == "["
        ):  # The same is true if it's the start of a new section, signalled by '['
            readMode = False
        if readMode is True:
            if (
                line[0] != ";" and currentLine.isspace() is False
            ):  # If the line isn't a comment
                if indexingEnd == "None":
                    importantLine = (currentLine.split())[
                        indexingStart:
                    ]  # Just split with a start point, and take the rest
                else:
                    importantLine = (currentLine.split())[
                        indexingStart:indexingEnd
                ...
                listArray.append(
                    importantLine
                )  # Append the important line to the list array
        if (
            splitString in currentLine
        ):  # Check at the end because we only want to start splitting after the string; if found, it'll split all further lines till it gets to white space
            readMode = True
    openedFile.close()
...
def readITP(itpPath):
    ...
    for (
        ...
        atomsList
    ):  # A simple bit of  neatening; make an associated atom list, as we don't care about the comments or punctuation
        atomNum = atom[0]
        atomType = atom[1]
        bondType = atom[4]
        bondTypeList.append(bondType)
        associationList = [atomNum, atomType, bondType]
        associatedAtomList.append(
            associationList
    ...
    dihedralsList = fetchList(
        itpPath, " PROPER DIHEDRAL", 0, 11
...
def makeBondingTypeAssociation(
    associations, featureList, featureAtomCount, improper=False
    ...
    for feature in featureList:  # for each feature (bond/angle/dihedral)
        intList = [
            int(i) for i in feature[0:featureAtomCount]
        ...
        for (
            ...
            intList
        ):  # For each atom in the feature (currently represented as a number)
            for (
                assoc
            ) in (
                associations
            ):  # We compare it against each of the atom number, atomtype, and bonding type associations
                atomNum = int(assoc[0])
                if (
                    atom == atomNum
                ):  # If the atom number of this feature matches that of the association, then append the bonding type of that association to the list
        ...
        if featureAtomCount == 4 and improper is True:
            bondingTypeList = bondingTypeList + [
                0.000,
                0.000,
                feature[-1],
                0.000,
...
def makeAllAssocciations(itpPath):
    ...
    for (
        ...
        impropersList
    ):  # We need to make some modifications, because the central atom definition is changed in LAMMPS from GROMACS
    ...
    totalBondList = makeBondingTypeAssociation(
        associationList, bondsList, 2
    )  # Call the makeBondingTypeAssociation function, getting the bond associations
    totalAnglesList = makeBondingTypeAssociation(
        associationList, anglesList, 3
...
def uniqueList(
    list,
):  # A pretty self-explanatory little function; it takes a list, and returns that list in order with any duplicates removed
    uniqueList = []
    for item in list:
...
def formatILFF(
    ...
    if (
        paramType == "bond"
    ):  # We have to format this differently for each type of parameter
        writeFile.write("\nBONDS\n")
        for param in paramList:
            equilBondLength = format(
                float(param[2]) * 10, ".3f"
            ...
            while len(param[1]) < 5:
                param[1] = param[1] + " "
            if (
                "H" in param[0][0] or "H" in param[1][0]
                ...
                if (
                    outputLine not in outputLines
                ):  # One last little check to ensure uniqueness; we put in the formatted line above in a list to check against, and then write it
        ...
        paramType == "angle"
    ):  # We repeat the 'bond' process for angles, with some minor changes
        ...
        for param in paramList:
            ...
            if (
                outputLine not in outputLines
            ):  # Also no need for the 'H' check; we're not freezing angles
                outputLines.append(outputLine)
                writeFile.write(outputLine)
    elif (
        paramType == "dihedral" or paramType == "improper"
        ...
        for param in paramList:
            dihedralConstant1 = str(
                format(abs(float(param[5])), ".4f")
            ...
            outputLine = (
                outputLine + dihedralConstant1
            )  # and add this string to the output line
            for (
                constant
            ) in (
                dihedralConstantList
            ):  # The other 3 have 10 characters before the end, so we repeat the process
        ...
        )  # A warning if this somehow happens, likely a typo
    writeFile.write(
        "\n"
...
def fetchNonBonded(
    ...
    for param in atomTypeList:
        if param[0] == atomtype:
            ...
            sigma = (
                float(param[-2]) * 10
            )  # These are coming in as nm, and we need them in Angstrom
            return [sigma, epsilon]


def makeFFNonBonded(
    atomList, atomTypeList, outputPath
    ...
    for atom in atomList:  # For each atom in the list of atom types
        ...
        while (
            len(atomName) < 6
        ):  # There should be 6 characters from the start of the atom name to the start of the bonding type; this ensures it
            atomName = atomName + " "
        ...
        if (
            float(atom[6]) >= 0
        ):  # Atomic charge is defined to 2 decimal places, with a slight change in output based on the presence of the '-' character
            atomicCharge = format(float(atom[6][1:]), ".4f")
        elif float(atom[6]) < 0:
            atomicCharge = format(float(atom[6]), ".4f")
        totalAtomicCharge += float(
            atomicCharge
        ...
        if (
            atomCount == len(atomList) and chargeDifference < 0.001 > -0.001
        ...
        elif atomCount == len(atomList) and (
            chargeDifference > 0.001 or chargeDifference < -0.001
        ):
            print(
                "You have relatively significant (>0.001) divergences in partial charge"
            )
        while len(atomicCharge) < 9:  # The spacing is corrected
            atomicCharge = " " + atomicCharge
        atomType = atom[1]
        ljParam = fetchNonBonded(
            atomType, atomTypeList
...
def fetchXYZ(
    ...
    for atom in atomsStart:  # For each atom taken from the lammps topology file
        ...
        for (
            ...
            atomsList
        ):  # For each atom in the list of atoms we already have, check if the atom numbers match
...
def renameMonomer_old(
    monomer, outName, atomNames
):  # This function, and renameXYZ_old are kept here for a little while in case they're needed; they function off of an input .xyz, instead of recreating an .xyz from the LAMMPS data file
...
def makeXYZ(
    inputFile, atomNames, outName, ffName
):  # A function to make the required .xyz file for fftool
    if os.path.isfile(
        outName
    ):  # If the file already exists, delete it; we don't want anything overlapping here
        os.remove(outName)
    fullResidueList = inputFile
    header = (
        str(len(fullResidueList)) + "\n " + ffName[:-3] + " " + ffName
    ...
    for line in fullResidueList:  # For each line in the full residue list
        xVar = str(
            format(float(line[1]), ".6f")
        )  # We define the coordinate to 6 decimal places
        while (
            len(xVar) < 19
        ):  # And adjust the spacing to make sure we've got it formatted correctly
            xVar = " " + xVar
        yVar = str(
            format(float(line[2]), ".6f")
...
def convertToIlFF(itpPath, outputPath, lammpsTopology, crystalOutput="placeholder"):
    if os.path.isfile(
        outputPath
    ):  # If the output already exists, delete it so the appending doesn't work very badly
        os.remove(outputPath)
    if (
        crystalOutput == "placeholder"
    ...
    associationList = makeAllAssocciations(
        itpPath
    ...
    makeXYZ(
        ...
        outName=crystalOutput,
        ffName=outputPath,
    )  # Make the XYZ required from the temporary file
    makeFFNonBonded(
        atomsList, atomTypeList, outputPath
    )  # Call the function that makes the nonbonded part of the FF file
    formatILFF(
        paramType="bond", paramList=bondParams, output=outputPath
...
parser = ArgumentParser(
    usage="convertLigParGen.py -g [.itp file] -l [.lmp file] -o [output name]",
...
parser.add_argument(
    ...
    "--gromacs",
    required=True,
    help="The required .itp file generated by LigParGen",
)
parser.add_argument(
    "-l",
    "--lammps",
...
parser.add_argument(
    "-o", "--output-ff", required=True, help="The name of the forcefield file generated"
)
parser.add_argument(
    "-xyz" "--xyzFile",
    required=False,
...
if args.xyz__xyzFile is None:
    convertToIlFF(
        itpPath=args.gromacs, outputPath=args.output_ff, lammpsTopology=args.lammps
    )
else:
    convertToIlFF(
        itpPath=args.gromacs,
        outputPath=args.output_ff,
        lammpsTopology=args.lammps,
        crystalOutput=args.xyz_xyzFile,
    )

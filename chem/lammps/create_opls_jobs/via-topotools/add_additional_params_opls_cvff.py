...
def get_args():
    ...
    parser.add_argument(
        ...
        "--polym",
        help=
        "polym.in file required by Polymatic. For this code to work, this must include a link command",
    )
    parser.add_argument(
        "-o",
        "--output",
        help=
        "Filename of the output file. If not given, the original LAMMPS datafile is overwritten",
    )
    ...
    for line in ff_dihs:
        for combo, types_that_exist in zip(all_combos, with_types_that_exist):
            ...
            if all(val != "X" for val in line[:4]):
                ...
                types_to_check = {
                    idx: val
                    for idx, val in enumerate(line[:4]) if val != "X"
                }
            ...
            if i == "\n" and first_newline_skipped:
                for new, val in enumerate(pairs_to_add, 1):
                    ...
                    df.insert(
                        ...
                        "{:<4} {:>10.3f} {:>9.5f}    # {}\n".format(
                            total,
                            float(val[2]) / 4.184, float(val[1]), val[0]),
                    )
    return df, total
            ...
            if i == "\n" and first_newline_skipped:
                for new, val in enumerate(bonds_to_add, 1):
                    ...
                    df.insert(
                        ...
                        "{:<4} {:>9.1f} {:>10.3f}    # {}-{}\n".format(
                            total,
                            float(val[3]) / (2 * 4.184), float(val[2]), val[0],
                            val[1]),
                    )
    return df, total
...
def add_angles(datafile, angles, original):
    df = datafile.copy()
    angles_to_add = [
        i for i in angles if f"{i[0]}-{i[1]}-{i[2]}" not in original
    ]
    found = False
    num = 0
    total = len(original)
            ...
            if i == "\n" and first_newline_skipped:
                for new, val in enumerate(angles_to_add, 1):
                    ...
                    df.insert(
                        ...
                        "{:<4} {:>9.1f} {:>10.3f}    # {}-{}-{}\n".format(
                            total,
                            float(val[4]) / (2 * 4.184), float(val[3]), val[0],
                            val[1], val[2]),
                    )
    return df, total
...
def add_dihs(datafile, dihs, original):
    df = datafile.copy()
    dihs_to_add = [
        i for i in dihs if f"{i[0]}-{i[1]}-{i[2]}-{i[3]}" not in original
    ]
    found = False
    num = 0
    total = len(original)
        ...
        if found:
            ...
            if i == "\n" and first_newline_skipped:
                for new, val in enumerate(dihs_to_add, 1):
                    total += 1
                    params = " ".join([str(float(i) / 4.184) for i in val[4:]])
                    df.insert(
                        idx - 1 + new,
                        "{:<4} {}  # {}-{}-{}-{}\n".format(
...
def add_imps(datafile, imps, original):
    df = datafile.copy()
    imps_to_add = [
        i for i in imps if f"{i[0]}-{i[1]}-{i[2]}-{i[3]}" not in original
    ]
    found = False
    num = 0
    total = len(original)
...
def main():
    args = get_args()
    atom_types_to_add, linkers = linker_atom_types(args.lammps,
                                                   ff=args.forcefield,
                                                   polym=args.polym)
    ...
    addn_angles = angles_from_ff(atom_types_to_add, linkers, args.forcefield,
                                 params["angles"])
    addn_dihs = dihs_from_ff(atom_types_to_add, linkers, args.forcefield,
                             params["dihs"])
    addn_imps = imps_from_ff(atom_types_to_add, linkers, args.forcefield,
                             params["imps"])
